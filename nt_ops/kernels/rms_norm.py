...
def _premake_rowwise(input_dtype=None, norm_size=None):
    # norm_size (Python int) is only used for shape_options upper_bound.
    # normalized_numel is passed as a runtime Tensor(0) scalar — 0-dim tensors
    # are skipped in the autotune inequality loop so they need no upper_bound.
    def _arrangement(input, weight, eps, output, normalized_numel):
    ...
    def _application(input, weight, eps, output, normalized_numel):
    ...
    tensors = (
        ...
        Tensor(0, dtype=ninetoothed.int64),  # normalized_numel runtime scalar
    )
