...
def arrangement(q, k, v, out, seqused_k, block_table, softmax_scale):
    ...
    # b, -> b | 1 -> b n g | 1
    seqused_k_arranged = seqused_k.tile((1,)).unsqueeze(1).unsqueeze(2).expand((-1, q_arranged.shape[1], q_arranged.shape[2]))
    # b k -> b 1 | 1 k -> b 1 1 | 1 k -> b n g | k
    block_table_arranged = block_table.tile((1, -1)).unsqueeze(2).expand((-1, q_arranged.shape[1], q_arranged.shape[2]))
    block_table_arranged.dtype = block_table_arranged.dtype.squeeze((1,))
...
def application(q, k, v, out, seqused_k, block_table, softmax_scale):
    ...
    len_seq_k = seqused_k
    block_size = block_table.shape[1]
...
def premake():
    ...
    tensors = (
        ...
        # k
        Tensor(5, jagged_dim=1, shape_options={"constexpr": True, "upper_bound": 128}),
        # v
        Tensor(5, jagged_dim=1, shape_options={"constexpr": True, "upper_bound": 128}),
        # out
        Tensor(5, jagged_dim=1, shape_options=shape_options),
